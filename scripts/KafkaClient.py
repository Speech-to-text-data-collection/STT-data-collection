...
logger = CreateLogger('KafkaClient', handlers=1)
...
class KafkaClient():
    def __init__(self, id: str, servers: list) -> None:
        try:
            ...
            logger.info('SUCCESSFULLY CREATED KafkaClient OBJECT INSTANCE.\nSERVERS:\n')
            for index, server in enumerate(self.kafka_servers):
                print(f'\t{index + 1} - {server}')

        except Exception as e:
            logger.exception("FAILED TO CREATE KafkaClient OBJECT INSTANCE")
    ...
    def create_admin_client(self) -> None:
        try:
            ...
            logger.info('SUCCESSFULLY CREATED A KAFKA ADMIN CLIENT')

        except Exception as e:
            logger.exception('FAILED TO CREATE A KAFKA ADMIN CLIENT')

    def get_topics(self) -> list:
        try:
            topics = self.admin_client.list_topics()

            logger.info('SUCCESSFULLY FETCHED KAFKA TOPICS')

            return topics

        except AttributeError:
            logger.error('INSTANTIATE A KAFKA ADMIN CLIENT FIRST USING THE create_admin_client METHOD')
        except Exception as e:
    ...
    def create_topic_type(self, name: str, partitions: int = 1, replication_factor: int = 1) -> NewTopic:
        try:
            ...
            logger.info('SUCCESSFULLY INSTANTIATED A TOPIC OBJECT')

            return topic
        
        except Exception as e:
            logger.exception("FAILED TO INSTANTIATE A TOPIC OBJECT")
    ...
    def create_producer(self, key_serializer=None, value_serializer=None, acks: int = 1, retries: int = 0, max_in_flight_requests_per_connection: int = 5):
        try:
            ...
            logger.info(f"SUCCESSFULLY CREATED A KAFKA PRODUCER")
        except Exception as e:
            logger.exception("FAILED TO CREATE A KAFKA PRODUCER")
    ...
    def send_data(self, topic_name: str, data_list: list, func=lambda x: x):
        try:
            ...
            logger.info(f"SUCCESSFULLY SENT {len(data_list)} DATA VALUES")

        except AttributeError:
            logger.error('INSTANTIATE A KAFKA PRODUCER FIRST USING THE create_producer METHOD')
    ...
    def create_consumer(self, topics: str, group_id: str, auto_commit: bool = True, offset: str = 'earliest', key_deserializer=None, value_deserializer=None, timeout: int = 20):
        try:
            ...
            logger.info(f"SUCCESSFULLY CREATED A KAFKA CONSUMER")

        except Exception as e:
            logger.exception("FAILED TO CREATE A KAFKA CONSUMER")
    ...
    def get_data(self, verbose:int=0):
        try:
            ...
            logger.info(f"SUCCESSFULLY FETCHED {len(return_data)} DATA VALUES")

            return return_data

        except AttributeError:
            logger.error('INSTANTIATE A KAFKA CONSUMER FIRST USING THE create_consumer METHOD')
